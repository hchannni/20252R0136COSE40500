...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        window_shape = (1, C, f_h, f_w)
        x_windows = view_as_windows(x, window_shape, step=1)
        x_windows = x_windows.squeeze(4).squeeze(1)

        out = torch.einsum('nhwcxy,ocxy->nohw', x_windows, self.W)
        out = out + self.b
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        window_shape = (1, 1, self.pool_size, self.pool_size)
        step = (1, 1, self.stride, self.stride)
        
        x_windows = view_as_windows(x, window_shape, step=step)
        x_windows_flat = x_windows.reshape(x_windows.size(0), x_windows.size(1), 
                                        x_windows.size(2), x_windows.size(3), -1)
        
        out, _ = torch.max(x_windows_flat, dim=-1)
